...
from app_balance.queue.per_op_reading import TaskOpAccountRole, TaskType, TaskTypeOp
...
class ResourceUsageRepo:
    ...
    async def record_op(
        ...
        *,
        ...
        """E6 — INSERT расхода одного op для multi-op идемпотентного пайплайна.

        В отличие от record_for_task (списывает все op попытки сразу), позволяет
        списывать ресурс пошагово — только за фактически выполняемые op, чтобы
        retry не дублировал INSERT за уже завершённые шаги (ТЗ §29).
        """
