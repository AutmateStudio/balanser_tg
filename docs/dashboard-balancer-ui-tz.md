...
> **Самодостаточность:** документ содержит **все требования к UI**, **полный справочник HTTP API** (§9) и **контракты данных очереди** (§9.5). Внешняя документация репозитория балансировщика **не требуется**.
...
| Статус | PG `status` + runtime (`connected`, `running`, `banned`, …) |
| В clump | `in_clump`, `parser_id`, `clump_name` |
| Каналов | `channel_count` / `max_channels` |
| Per-op ресурс | остаток RPH по каждому **op** (из `accounts.per_op[]`, §0.5 утверждённой схемы) |
| % ресурса | `available_resource_percent` (сводка или min по ops) |
| Последняя ошибка | `last_error`, flood/ban reason |
...
- Просмотр **cooldown_until** / flood remaining (§6.1, §7).
...
3. **Аккаунты** показывает метрики §26.3, список аккаунтов с per-op остатком ресурса и ошибками; работает QR-авторизация и polling статуса.
...
| Индикатор | «Обновлено: {generated_at локальное время}» |
...
| Аккаунты | Expand row: per-op badges + `account-channels` lazy load |
...
| Решение | Детали |
|---------|--------|
| Расположение | `src/schemas/balancer/` — Zod-схемы `MetricsResponse`, `AccountFullSummary`, `TaskQueueItem`, … |
| Синхронизация | Ручная по §9.5 + §11; CI script `pnpm run sync:balancer-openapi` (optional, Phase 2) |
