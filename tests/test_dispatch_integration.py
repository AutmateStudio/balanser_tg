...
class _FailOnceAdapter(MockTaskAdapter):
    """E4: падает на первом execute, затем успех."""
...
async def _run_worker_until_status(
    ...
    try:
        await asyncio.wait_for(
            _wait_until_status(task_id, expected=expected, timeout=timeout),
            timeout=timeout,
        )
    finally:
        worker.stop()
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_e4_success_creates_one_task_attempt(clean_queue) -> None:
    """E4: 1 execute → 1 строка task_attempts со status=success."""
    account_id, _ = await _insert_account(session_suffix="e4_ok")
    repo = TaskQueueRepo()
    task_id = await _enqueue(account_id=account_id)
    claimed = await repo.claim_by_id(task_id, locked_by="e4-success-it")
    if claimed is None:
        pytest.skip(
            "фоновый queue-worker забрал задачу до claim — "
            "E4 wiring покрыт unit-тестами"
        )
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_e4_fail_retry_success_creates_two_attempts(clean_queue) -> None:
    """E4 (PG): fail → retry → success через dispatch; 2 строки attempt_number=[1,2]."""
    account_id, _ = await _insert_account(session_suffix="e4_retry")
    mock = _FailOnceAdapter()
    repo = TaskQueueRepo()
    task_id = await _enqueue(account_id=account_id)
    claimed1 = await repo.claim_by_id(task_id, locked_by="e4-retry-1")
    if claimed1 is None:
        pytest.skip(
            "фоновый queue-worker забрал задачу до claim — "
    ...
    assert [int(a["attempt_number"]) for a in attempts] == [1, 2]
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_e4_postpone_creates_no_task_attempt(clean_queue) -> None:
    """E4: postpone без execute → task_attempts пуст."""
    account_id, _ = await _insert_account(session_suffix="e4_postpone")
    holder_task_id = await _enqueue(priority=_HOLDER_PRIORITY)
    async with db.acquire() as conn:
        await conn.execute(
            """
    ...
    repo = TaskQueueRepo()
    task_id = await _enqueue(account_id=account_id)
    claimed = await repo.claim_by_id(task_id, locked_by="e4-postpone-it")
