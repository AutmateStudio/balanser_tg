...
async def test_dispatch_records_usage_before_adapter() -> None:
    ...
    attempt_ids = {row.task_attempt_id for row in usage.inserts}
    assert len(attempt_ids) == 1
    assert next(iter(attempt_ids)) == 1


@pytest.mark.asyncio
async def test_dispatch_passes_same_task_attempt_id_to_all_ops() -> None:
    """E4+D5: все INSERT usage одной попытки ссылаются на один task_attempt_id."""
    usage = RecordingUsageRepo()
    ops = _parser_add_channel_ops()
    dispatcher = _dispatcher(
        _fake_queue(),
    ...
    await dispatcher.dispatch(_claimed(58, account_id=42))

    assert len(usage.inserts) == 4
    attempt_ids = {row.task_attempt_id for row in usage.inserts}
    assert len(attempt_ids) == 1
    assert next(iter(attempt_ids)) is not None
...
async def test_dispatch_adapter_error_usage_not_rolled_back() -> None:
    ...
    assert usage.inserts[0].task_attempt_id == 1
...
async def d5_ctx(pg_pool):
    ...
    claimed = await repo.claim_by_id(
        enqueue.task_id, locked_by=f"{_PREFIX}fixture"
    )
    if claimed is None:
        pytest.skip(
            "фоновый queue-worker забрал задачу до claim — проверьте изоляцию PG"
        )
...
async def test_dispatch_parser_add_channel_writes_usage(d5_ctx) -> None:
    ...
    async with db.acquire() as conn:
        ...
        usage_rows = await conn.fetch(
            """
            SELECT aru.task_attempt_id, ta.id AS attempt_id, ta.attempt_number, ta.status
            FROM account_resource_usage aru
            JOIN task_attempts ta ON ta.id = aru.task_attempt_id
            WHERE aru.task_id = $1
            """,
            ctx["task_id"],
        )

    assert len(usage_rows) == len(expected)
    attempt_ids = {int(r["task_attempt_id"]) for r in usage_rows}
    assert len(attempt_ids) == 1
    assert usage_rows[0]["attempt_number"] == 1
...
async def test_dispatch_execute_failure_usage_persists(d5_ctx) -> None:
    ...
    async with db.acquire() as conn:
        ...
        null_attempt_ids = await conn.fetchval(
            """
            SELECT COUNT(*) FROM account_resource_usage
            WHERE task_id = $1 AND task_attempt_id IS NULL
            """,
            ctx["task_id"],
        )
        attempt_row = await conn.fetchrow(
            """
            SELECT id, attempt_number, status FROM task_attempts
            WHERE task_id = $1
            """,
            ctx["task_id"],
        )
    ...
    assert null_attempt_ids == 0
    assert attempt_row is not None
    assert attempt_row["status"] == "error"
