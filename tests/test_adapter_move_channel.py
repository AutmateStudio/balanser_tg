...
async def test_move_source_account_not_found_raises() -> None:
    ...
    with pytest.raises(PermanentError, match="account_not_found:10") as exc_info:
        await execute_task(
            _move_claimed(),
...
async def test_move_missing_source_account_id_raises() -> None:
    ...
    with pytest.raises(PermanentError, match="missing dual account ids"):
        await execute_task(
            task,
            account=_account(20, "/tgt"),
...
async def test_move_missing_parser_id_raises() -> None:
    ...
    with pytest.raises(PermanentError, match="missing parser_id"):
        await execute_task(
            task,
            account=_account(20, "/tgt"),
            clump_getter=lambda _pid: FakeClump(),
...
async def test_move_missing_channel_ref_raises() -> None:
    ...
    with pytest.raises(PermanentError, match="missing channel_ref"):
        await execute_task(
            task,
            account=_account(20, "/tgt"),
            clump_getter=lambda _pid: FakeClump(),
...
async def test_unsupported_task_type_raises() -> None:
    ...
    with pytest.raises(PermanentError, match="unknown_type"):
        await execute_task(
            task,
            account=_account(20, "/tgt"),
            clump_getter=lambda _pid: FakeClump(),
...
async def test_dispatch_move_channel_clump_error_releases_both() -> None:
    ...
    assert accounts.released == [10, 20]
    assert queue.failed == [(22, "clump_error")]


@pytest.mark.asyncio
async def test_dispatch_move_channel_missing_parser_id_fails() -> None:
    clump = FakeClump()
    accounts = SessionNamedDualAccounts()
    adapter = ClumpTaskAdapter(
        clump_getter=lambda _pid: clump,
    ...
    assert accounts.released == [10, 20]
    assert queue.permanent_failed == [(23, "invalid_payload")]
    assert queue.failed == []
