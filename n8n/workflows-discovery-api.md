...
- **Телеграм поиск** — единый `POST /discover` (async, upsert в `source_channels` на стороне API) + `add-channel-by-link` по seed-фразам; `/discover-groups` deprecated.
...
- **ID:** `Cno7xg0nQg8D`
- **Тип запуска:** sub-workflow
- **Что делает:** Sub-workflow: по seed-фразам ищет Telegram-каналы/группы через Discovery API (`POST /discover` async), upsert в `source_channels` выполняет API (не n8n).
...
- **Сторонние API / интеграции:**
  - **Lidogen Discovery API:**
    - `POST /discovery-api/discover` (async, poll `GET /parser/queue/tasks/{task_id}`)
    - `POST /discovery-api/add-channel-by-link`
    - `POST /discovery-api/auth/qr`
  - **Telegram:**
    - t.me
  - **PostgreSQL:**
    - `source_channels` — только чтение/dedup (upsert-ноды убрать после миграции)

### Миграция на unified `/discover` (после деплоя API)

1. Заменить пару вызовов `discover` + `discover-groups` одним `POST /discover?async=true`.
2. Poll `GET /discovery-api/parser/queue/tasks/{task_id}` до `status=done`.
3. Удалить postgres-ноды INSERT/UPDATE в `source_channels` — данные в `payload.result.persist` и в таблице.
4. Broadcast без discussion не попадают в БД (`persist.skipped_no_discussion`).
5. Целевой файл для правок: `телеграм-поиск-Cno7xg0nQg8D-newapi.json`.

Подробнее: [`docs/discovery-api-endpoints.md`](../docs/discovery-api-endpoints.md#post-discovery-apidiscover-groups).
